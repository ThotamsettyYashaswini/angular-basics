...
@Component({
  selector: 'app-allocating-mutation-dummy',
  templateUrl: './allocating-mutation-dummy.component.html',
  styles: []
})
export class AllocatingMutationDummyComponent implements OnInit {
  savebutton: any = 'Save';
  ...
  getProceedingNo() {
    debugger;
    this._plotcreationservices.getProceedingNo().subscribe(json => {
  ...
  proceedingNoChange(event) {
    ...
    this._plotcreationservices.bindMutation(event.pproceedingno, event.pmutationid).subscribe(res => {
  ...
  selectSurvey(dataList) {
    ...
    this.mutationAddToGridForm.controls.secondarylandareauomname.setValue('');
    this.mutationAddToGridForm.controls.secondarylandareauom.setValue('');
    this._plotcreationservices.getMutationDocuments(dataList.pproceedingno, dataList.psurveyno, dataList.pmutationdetailsid, dataList.pvillagemandal, dataList.pregistereddocument).subscribe(result => {
      ...
      this.mutationDocumentsList = this.mutationDocumentsList.map(obj => {
        obj.pprimarymutationlandarea = 0;
        obj.pprimarymutationlandareaValueUOM = '';
        obj.psecondarymutationlandarea = 0;
        obj.psecondarymutationlandareauomname = '';

        obj.ptotalmutationlandareainacres = '0 Acre';
        obj.ptotalmutationlandareainsqyds = 0;
  ...
  ChangeInput(event, dataItem, rowIndex) {
    ...
    this.mutationDocumentsList[i].pprimarymutationlandarea = this.mutationAddToGridForm.controls.primaryLandareaValue.value;

    this.mutationDocumentsList[i].pprimarymutationlandareaValueUOM = this.mutationAddToGridForm.controls.primarylandareauomname.value;

    this.mutationDocumentsList[i].psecondarymutationlandarea = this.mutationAddToGridForm.controls.secondarylandareavalue.value;
    this.mutationDocumentsList[i].psecondarymutationlandareauomname = this.mutationAddToGridForm.controls.secondarylandareauomname.value;

    this.mutationDocumentsList[i].ptotalmutationlandareainacres = this.totalArea;
  ...
  saveMutationDocument() {
    ...
    const hasInvalidDoc = this.mutationDocumentsList.some(doc => {
      return (
        (doc.pprimarymutationlandarea == 0 || doc.pprimarymutationlandarea === "0") &&
        (doc.psecondarymutationlandarea == 0 || doc.psecondarymutationlandarea === "0") &&
        doc.pprimarymutationlandareaValueUOM === '' &&
        doc.psecondarymutationlandareauomname === ''
      );
    });

    if (hasInvalidDoc) {
      this._commonservice.showWarningMessage('Invalid mutation document found. Please check land area and units.');
      return;
    }

    let isvalid = true;
    let data = {
      "mutationDocumentDetailslist": this.mutationDocumentsList
    }
    console.log(JSON.stringify(data));
    let formData = JSON.stringify(data);

    if (this.checkValidations(this.mutationAllocatingForm, isvalid)) {
      this._plotcreationservices.saveMutationDocument(formData).subscribe(res => {
        if (res) {
