...
declare const $: any;
@Component({
  selector: 'app-conversion-document-details',
  templateUrl: './conversion-document-details.component.html',
  styles: []
})
export class ConversionDocumentDetailsComponent implements OnInit {
  ...
  constructor(private _commonService: CommonService, private fIIndividualService: FIIndividualService, private _companyconfigservice: CompanyconfigService, private _landbankservice: LandBankService,private http:HttpClient, private datePipe: DatePipe, private _plotcreationservices:PlotcreationService) { }
  ...
  getDocmentDetails(){
    this._plotcreationservices.getConvertionDocumentType(1).subscribe(response => {
  ...
  companyconfigdocumentcontrols(): FormGroup {
    return new FormGroup({
      ...
      pImagepath: new FormControl(''),
      conversionNo: new FormControl(''),
      conversionId: new FormControl(''),
  ...
  getProceedingNo() {
    debugger;
    this._plotcreationservices.getMutationProceedingNosDoc().subscribe(json => {
      this.mutationList = json;
    })
  }
  mutationChangeList(event){
    debugger;
    this.companyconfigdocumentsform['controls']['conversionId'].setValue(event.pconvertionid);  
    this.getMutationDocdetails(event.pconvertionid);  
    this.GetmutationUploadSurveyno(event.pproceedingno)    
    this.proceedingNum = event.pproceedingno;
  }

  GetmutationUploadSurveyno(pproceedingno){
    this._plotcreationservices.GetconvertionUploadSurveyno(pproceedingno).subscribe(json => {
      this.conProNo = json;
      console.log(this.existingDocList);      
    })
  }

  getMutationDocdetails(pconvertionid){
    this._plotcreationservices.getConvertionDocdetails(pconvertionid).subscribe(json => {
  ...
  changeSurveyNo(event){
    debugger
    this._plotcreationservices.GetconvertionUploadDocno(this.proceedingNum,event.psurveyno).subscribe(json => {
      this.docNoList = json;
      console.log(this.existingDocList);      
    })
  }
  ...
  documentDelete(dataItem){
    ...
    console.log(data1);    
    this.fIIndividualService.deleteDocumentDetails(data1).subscribe(res => {      
      this.getMutationDocdetails(dataItem.pmutationid);
  ...
  pIdsProof_Change($event: any) {
    ...
    // if (pDocumentId && pDocumentId != '') {
      ...
      this._plotcreationservices.getMutationDocumentType(pDocumentId).subscribe(response => {
        if (response.length != 0) {
          debugger
          this.kycDocumentType = response;
        }
...
var extention = event.target.value.substring(event.target.value.lastIndexOf('.') + 1);
 ...
 let gridDataAddre =    this.gridData.map(obj => {
      obj.transactionno = this.companyconfigdocumentsform['controls']['conversionId'].value;
      obj.typeid = this.companyconfigdocumentsform['controls']['conversionId'].value;
    ...
    this._plotcreationservices.saveConvertionDocumentDetails(data).subscribe(res=>{
         if(res){
          this._commonService.showInfoMessage('Documents Saved Successfully');
          this.gridData = [];
          this.existingDocList = [];
          this.companyconfigdocumentsform['controls']['conversionNo'].setValue('');
         }
    })
  }
