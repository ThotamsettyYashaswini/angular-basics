...
@Component({
  selector: 'app-allocating-mutation',
  templateUrl: './allocating-mutation.component.html',
  styles: []
})
export class AllocatingMutationComponent implements OnInit {
  savebutton: any = 'Save';
  ...
  selectSurvey(dataList){
    ...
    this._plotcreationservices.getMutationDocuments(dataList.pproceedingno,dataList.psurveyno,dataList.pmutationdetailsid,dataList.pvillagemandal,dataList.pregistereddocument).subscribe(result => {
      ...
      if(this.mutationDocumentsList.length == 0){
        this._commonservice.showInfoMessage('No Data To Show');
      }
      ...
      this.mutationDocumentsList = this.mutationDocumentsList.map(obj => {
        ...
        obj.ptotalmutationlandareainacres = '';
  ...
  ChangeInput(mutationDocList,event, index){
    debugger;
    let i = index;
