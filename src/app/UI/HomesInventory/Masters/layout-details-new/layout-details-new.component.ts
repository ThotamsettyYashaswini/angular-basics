...
@Component({
  selector: 'app-layout-details-new',
  templateUrl: './layout-details-new.component.html',
  styles: []
})
export class LayoutDetailsNewComponent implements OnInit {
  savebutton: any = 'Save';
  ...
  sumForSqyareYards: any;
  gridForCent: any;
  assignedSecondaryUOMId: any;
  ...
  addDetailsToGrid() {
    ...
    let splitArea = this.totalExtentInAcres.split(' ');
    if(splitArea[3].toUpperCase() == 'GUNTAS' ){
      this.assignedSecondaryUOMId =  7;
    }
    else if(splitArea[3].toUpperCase() == 'CENTS' ){
      this.assignedSecondaryUOMId =  2;
    }
    else{
      this.assignedSecondaryUOMId =  5;
    }
    ...
    if (this.checkValidations(this.layoutDetailsAddToGridForm, isvalid)) {
      let data = {
        ...
        'pprimarylandarea': splitArea[0],
        'psecondarylandarea': splitArea[2],
        'pproceedingno': this.layoutDetailsForm.controls.proceedingNo.value,
        'pproceedingdate': pDateGrid,
        "pprimarylandareauomid":1,
        "psecondarylandareauomid":this.assignedSecondaryUOMId,
         'pextentpsecondarylandarea': this.totalExtentInSquareYards,
        'pextentpprimarylandarea': this.totalExtentInAcres,
        "ptotallandareainacres": this.totalExtentInAcres,
        "ptotallandareainsqyds": this.totalExtentInSquareYards,
  ...
  secondaryUom_ChangeRoads($event: any): void {
    ...
    let secondarylandareauomnameParkk = this.layoutDetailsForm.controls.secondarylandareauomnamePark.value;
    let captilizeSecoundaryPARK = secondarylandareauomnameParkk.toUpperCase();
    if(this.captilizeGridValue == 'GUNTAS' && captilizeSecoundaryRoadArea == 'CENT' || this.captilizeGridValue == 'GUNTAS' && captilizeSecoundaryPARK == 'CENT'){
      let splitSecondaryValue = this.totalExtentAreaInACres.split(' ');
      let acreValue = splitSecondaryValue[0];
        let acre = splitSecondaryValue[1];
        let guntaVlaue = splitSecondaryValue[2];
        let gunta = splitSecondaryValue[3];
        let netPlotArea: any = this.convertToAcreAndCent(this.totalExtentAreaInACres)

      console.log(`Net Plot Area: ${netPlotArea.acres} Acre ${netPlotArea.cents} Cent`);
      this.gridForCent = `${netPlotArea.acres} Acre ${netPlotArea.cents} Cent`
      let primaryLandareaValueRoad = +this.layoutDetailsForm.controls.primaryLandareaValueRoads.value;
      let secondarylandareavalueRoad = +this.layoutDetailsForm.controls.secondarylandareavalueRoads.value;
      let primarylandareauomRoads =  this.layoutDetailsForm.controls.primarylandareauomRoads.value;
      let secondarylandareauomRoads =  this.layoutDetailsForm.controls.secondarylandareauomRoads.value;
      let primarylandareauomnameRoad =   this.layoutDetailsForm.controls.primarylandareauomnameRoads.value;
      let secondarylandareauomnameRoad =  this.layoutDetailsForm.controls.secondarylandareauomnameRoads.value;

      let addedvalue = primaryLandareaValueRoad + ' ' + primarylandareauomnameRoad + ' ' + secondarylandareavalueRoad + ' ' + secondarylandareauomnameRoad; 
    ...
    const parkArea = this.convertToAcresToCents(primaryLandareaValuePark, secondarylandareavaluePark);
      ...
      this.totalAreaInAcres = parkArea + roadsArea + roadAffectedArea + othersArea;

      // Calculate total Acres and Cents
      this.totalAcres = Math.floor(this.totalAreaInAcres);  // Get integer part (Acres)
      this.totalCents = ((this.totalAreaInAcres - this.totalAcres) * 100).toFixed(2);
      this.totalFinalAreas = this.totalAcres + ' ' + primarylandareauomnameRoads + ' ' + this.totalCents + ' ' + secondarylandareauomnameRoads;
  ...
  abc1() {
    ...
    if (secondae[3] == 'Guntas' && captilizeSecoundaryRoadArea == 'CENT' || secondae[3] == 'Guntas' && captilizeSecoundaryPARK == 'CENT') {

      // Extract the numerical values of Acres and Cents from the strings
      let [gridAcres, gridCents] = this.gridForCent.match(/\d+(\.\d+)?/g).map(Number);
      let [areaAcres, areaCents] = this.totalFinalAreas.match(/\d+(\.\d+)?/g).map(Number);
...
let splitSecondary = this.layoutDetailslist[0].pextentpprimarylandarea.split(' ');
    ...
    const parkArea = this.convertToAcresToCents(primaryLandareaValuePark, secondarylandareavaluePark);
      ...
      this.totalAcres = Math.floor(this.totalAreaInAcres);  // Get integer part (Acres)
      let totCents = 
      this.totalCents = ((this.totalAreaInAcres - this.totalAcres) * 100).toFixed(2);  // Get fractional part (Cents)
    ...
    if(this.entryEnable){
    const result = this.convertSqYardToAcreAndYard(this.sumForSqyareYards);
    console.log(`Acres: ${result.acres}, Remaining Square Yards: ${result.remainingSqYards}`);

    this.netPlotAreaFor = `${result.acres} Acres  ${result.remainingSqYards} Square Yards`
    this.afterCalSqYds = (this.sumForSqyareYards).toFixed(2) + ' ' + 'Square Yards';
    }
  ...
  saveLayoutDetails() {
    ...
    if(this.entryEnable == true){
    this.layoutDetailsForm.controls.secondarylandareavalueRoads.setValue(this.layoutDetailsForm.controls.roadAreaInSqyds.value);

    this.layoutDetailsForm.controls.secondarylandareavaluePark.setValue(this.layoutDetailsForm.controls.parkAreaInSqyds.value);

    this.layoutDetailsForm.controls.secondarylandareavalueRoadAffected.setValue(this.layoutDetailsForm.controls.roadAffectedAreaInSqyds.value);

    this.layoutDetailsForm.controls.secondarylandareavalueOthers.setValue(this.layoutDetailsForm.controls.othersAreaInSqyds.value);
    }
    let data = {
      "pproceedingno": this.layoutDetailsForm.controls.proceedingNo.value,
    ...
    if (this.checkValidations(this.layoutDetailsForm, isvalid)) {
      this._plotcreationservices.savelayout(data).subscribe(res => {
        if (res) {
          ...
          this.router.navigate(["/LayoutDetailsView"])
        }
      })
    }
  }
  ...
  clearLayoutDetails() {
    ...
    this.layoutDetailsForm.controls.secondarylandareavalueOthers.setValue('');
    this.layoutDetailsForm.controls.secondarylandareavalueTypeofDeed.setValue('');
    this.layoutDetailsForm.controls.roads.setValue(0);
    this.layoutDetailsForm.controls.park.setValue(0);
    this.layoutDetailsForm.controls.roadAffected.setValue(0);
    this.layoutDetailsForm.controls.description.setValue('');
    this.layoutDetailsForm.controls.others.setValue(0);
    this.layoutDetailsForm.controls.netArea.setValue(0);
    ...
    this.layoutDetailsForm.controls.roadAreaInSqyds.setValue('');
    this.layoutDetailsForm.controls.parkAreaInSqyds.setValue('');
    this.layoutDetailsForm.controls.roadAffectedAreaInSqyds.setValue('');
    this.layoutDetailsForm.controls.othersAreaInSqyds.setValue('');
    this.entryEnable = false;
    this.entryNo();
    this.ngOnInit();
