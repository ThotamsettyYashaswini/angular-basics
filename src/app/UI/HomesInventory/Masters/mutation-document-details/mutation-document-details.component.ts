...
declare const $: any;
@Component({
  selector: 'app-mutation-document-details',
  templateUrl: './mutation-document-details.component.html',
  styles: []
})
export class MutationDocumentDetailsComponent implements OnInit {
  ...
  getDocmentDetails(){
    this._plotcreationservices.getMutationDocumentType(1).subscribe(response => {
  ...
  companyconfigdocumentcontrols(): FormGroup {
    return new FormGroup({
      ...
      mutationNo: new FormControl(''),
      mutationId: new FormControl(''),
  ...
  getProceedingNo() {
    debugger;
    this._plotcreationservices.getProceedingNosDoc().subscribe(json => {
      this.mutationList = json;
    })
  }
  mutationChangeList(event){
    debugger;
    this.companyconfigdocumentsform['controls']['mutationId'].setValue(event.pmutationid);  
    this.getMutationDocdetails(event.pmutationid);  
  ...
  GetmutationUploadSurveyno(pproceedingno){
    this._plotcreationservices.GetmutationUploadSurveyno(pproceedingno).subscribe(json => {
      this.mutProNo = json;
  ...
  changeSurveyNo(event){
    debugger
    this._plotcreationservices.GetmutationUploadDocno(this.proceedingNum,event.psurveyno).subscribe(json => {
      this.docNoList = json;
      console.log(this.existingDocList);      
    })
  }

  getMutationDocdetails(pmutationid){
    this._plotcreationservices.getMutationDocdetails(pmutationid).subscribe(json => {
...
var extention = event.target.value.substring(event.target.value.lastIndexOf('.') + 1);
 ...
 let gridDataAddre =    this.gridData.map(obj => {
      obj.transactionno = this.companyconfigdocumentsform['controls']['mutationId'].value;
      obj.typeid = this.companyconfigdocumentsform['controls']['mutationId'].value;
    ...
    this._plotcreationservices.saveMutationDocumentDetails(data).subscribe(res=>{
         if(res){
          this._commonService.showInfoMessage('Documents Saved Successfully');
          this.gridData = [];
          this.existingDocList = [];
          this.companyconfigdocumentsform['controls']['mutationNo'].setValue('');
         }
    })
  }
