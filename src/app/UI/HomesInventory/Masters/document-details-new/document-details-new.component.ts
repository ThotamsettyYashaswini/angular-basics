...
declare const $: any;
@Component({
  selector: 'app-document-details-new',
  templateUrl: './document-details-new.component.html',
  styles: []
})

export class DocumentDetailsNewComponent implements OnInit {
  ...
  layoutnameChange(event){
    debugger;
    this.companyconfigdocumentsform['controls']['LayoutId'].setValue(event.plandbankid);  
    this.getLandDocDetails(event.plandbankid);      
  }

  getLandDocDetails(landbankid){
    this.fIIndividualService.GetlandDocdetails(landbankid).subscribe(json => {
  ...
  documentDelete(dataItem){
    ...
    this.fIIndividualService.deleteDocumentDetails(data1).subscribe(res => {      
      this.getLandDocDetails(dataItem.plandbankid);
...
var extention = event.target.value.substring(event.target.value.lastIndexOf('.') + 1);
 ...
 let gridDataAddre =    this.gridData.map(obj => {
    ...
    this.fIIndividualService.SaveDocumentDetails(data).subscribe(res=>{
         if(res){
