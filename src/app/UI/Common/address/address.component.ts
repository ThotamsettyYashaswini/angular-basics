...
export class AddressComponent implements OnInit {
  ...
  bindingdata(data) {
    debugger
    if ((data[0].latitude) == "") {
      this.addressForm.controls.latitude.clearValidators();
      this.addressForm.controls.latitude.updateValueAndValidity();
    }
    else {
      this.addressForm.controls.latitude.setValue(data[0].latitude)
    }
    if ((data[0].longitude) == "") {
      this.addressForm.controls.longitude.clearValidators();
      this.addressForm.controls.longitude.updateValueAndValidity();
    }
    else {
      this.addressForm.controls.longitude.setValue(data[0].longitude)
    }
    if ((data[0].pAddress1) == "") {
  ...
  bindingcompanydataEdit(data) {
    ...
    this.addressForm.controls.longitude.setValue(data[0].longitude);
    debugger;
    if(data[0].ptypeofoperation=='UPDATE'){
       this.addressForm.controls.ptypeofoperation.setValue('UPDATE');
    }
    else{
       this.addressForm.controls.ptypeofoperation.setValue('CREATE');
    }
  ...
  /**<-------(start) binding company creation address data to form (start)------> */
  bindingcompanycreationdata(data) {
    ...
    this.addressForm.controls.longitude.setValue(data.longitude);
    debugger;
    if(data.ptypeofoperation=='UPDATE'|| data.ptypeofoperation=='OLD'){
     this.addressForm.controls.ptypeofoperation.setValue('UPDATE')
    }
    else{
      this.addressForm.controls.ptypeofoperation.setValue('CREATE')
    ...
    this.addressForm.controls.pRecordid.setValue(data.addressid)

  }
  /**<-------(end) binding company creation address data to form (end)------> */
  ...
  editdata(data, formname) {
    ...
    else if (this.Form == "Company Creation") {
      this.addressForm.controls.pRecordid.setValue(data.addressid)
      this.bindingcompanycreationdata(data)
    }
    else if(this.Form=='Branch Creation'){
      this.addressForm.controls.pRecordid.setValue(data.addressid)
      this.bindingbranchcreationdata(data)
    }
  }
  /**<-------(end) update data of address form (append to form) (end)------> */

  /**<-------(start) checking validations based on form group (start)------> */
  bindingbranchcreationdata(data) {
    ...
    this.addressForm.controls.pRecordid.setValue(data.addressid);
    this.addressForm.controls.addressid.setValue(data.addressid);
  ...
  pCountry_Change($event: any): void {
    ...
    const countryid = $event.target.value;
      ...
      this.addressForm['controls']['pState'].setValue('');
      this.addressForm['controls']['pDistrict'].setValue('');
      this.addressformErrorMessage.pState=null;
