...
export class NavigationComponent implements OnInit, OnDestroy {
  ...
  public routdata: any;
  constructor(
    private _DefaultProfileImageService: DefaultProfileImageService,
    ...
    this._userService.GetUserForms(user.pUserName).subscribe((data) => {
      ...
      for (let i = 0; this.UsersForms.length > i; i++) {
        for (let j = 0; this.UsersForms[i]["lstSubModuleDTO"].length > j; j++) {
          for (
            let x = 0;
            this.UsersForms[i]["lstSubModuleDTO"][j]["functionsDTOList"]
              .length > x;
            x++
          ) {
            this.routdata.push(
              this.UsersForms[i]["lstSubModuleDTO"][j]["functionsDTOList"][x]
            );
          }
        }
      }
      this.routdata.sort((a, b) =>
        a.pFunctionName.localeCompare(b.pFunctionName)
      );
  ...
  pathChange(event) {
    debugger;
    // this._UsersService._Setclickurldata(event);
    ...
    let MenuNamedata = this.UsersForms.filter(
      (UsersArray) => UsersArray.pmoduleid == event["pmoduleid"]
    );
    let SubMenuNamedata = MenuNamedata[0]["lstSubModuleDTO"];
    let subMenuNamedata = SubMenuNamedata.filter(
      (UserssubArray) => UserssubArray.psubmoduleid == event["psubmoduleid"]
    );
  ...
  contactclick() {
    ...
    let MenuNamedata = this.routdata.filter(
      (UsersArrayName) => UsersArrayName.pFunctionName == this.MenuName
    );
