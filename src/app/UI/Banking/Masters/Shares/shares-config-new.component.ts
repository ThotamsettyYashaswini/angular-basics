...
export class SharesConfigNewComponent implements OnInit {
  ...
  constructor(private toastr: ToastrService, private shareconfigservice: ShareconfigService, private _commonService: CommonService, private formbuilder: FormBuilder, private router: Router) { }
...
this.GetSetTab1data();
  ...
  public SaveShareConfig() {
    ...
    if (this.sharenamecode.checkValidations(this.sharenamecode.CompanyNameCodeForm, isValid)) {
      this.shareconfigservice.CheckDuplicateShareName(this.sharenamecode.Duplicatesharename).subscribe(res => {
        this.duplicate = res
        debugger
        if (this.duplicate == 0) {
