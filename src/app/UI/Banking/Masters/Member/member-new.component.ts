...
import { FiKycandidentificationComponent } from 'src/app/UI/Loans/Transactions/FIIndividual/fi-kycandidentification.component';
import { FiReferencesComponent } from 'src/app/UI/Loans/Transactions/FIIndividual/fi-references.component';
import { FiPersonaldetailsComponent } from 'src/app/UI/Loans/Transactions/FIIndividual/fi-personaldetails.component';
...
export class MemberNewComponent implements OnInit {
  ...
  constructor(private _fb: FormBuilder, private zone: NgZone, private _MemberService: MemberService,
  ...
  ReferealComisionNotApplicableForLoan: Boolean
  ...
  PersonDetails(data, conferenceId) {
    ...
    this._MemberService.CheckContactDuplicate(data.pReferenceId).subscribe(result => {
      ...
      if (this.FimemCount == 0) {
        ...
        if (multicolumncombobox) {
          multicolumncombobox.value("")
          this.toaster.info("Selected Contact Already Exists")
        }
  ...
  ReferalComisionNotApplicable(event) {
    debugger
    let checked = event.target.checked;
    if (checked == true) {
      this.ReferealComisionNotApplicableForLoan = true;
